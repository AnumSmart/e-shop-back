...
// Интерфейс, по которому будут обращаться внешние сервисы
...
func (r *UserRepository) AddUser(ctx context.Context, email, hashedPass string) error {
	...
	query := `INSERT INTO users (email, hashed_pass) VALUES ($1, $2) ON CONFLICT (email) DO NOTHING`
	res, err := r.Database.GetPool().Exec(ctx, query, newUser.Email, newUser.HashPass)
	if err != nil {
